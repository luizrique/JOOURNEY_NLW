...
from src.controllers.participant_confirmed import ParticipantConfirmed
...
from src.models.repositories.participants_repository import PaticipantsRepository
...
def invite_to_trip(tripId):
    conn = db_connection_handler.get_connection()
    participants_repository = PaticipantsRepository(conn)
    emails_repository = EmailsToInviteRepository(conn)
...
def confirm_participant(participantId):
    conn = db_connection_handler.get_connection()
    participants_repository = PaticipantsRepository(conn)
    controller = ParticipantConfirmed(participants_repository)
